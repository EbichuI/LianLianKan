 ...
 * @return size个图片ID的集合
 */
NSMutableArray* agetRandomValues(NSArray* sourceValues , NSInteger size)
{
	// 创建结果集合
 ...
 * @return size个图片ID的集合
 */
NSArray* agetPlayValues(NSInteger size)
{
	if (size % 2 != 0)
	{
		// 如果该数除2有余数，将size加1
	...
	NSMutableArray* aplayImageValues = agetRandomValues(aimageValues() , size / 2);
	// 将playImageValues集合的元素增加一倍（保证所有图片都有与之配对的图片）
	[aplayImageValues addObjectsFromArray: aplayImageValues];
	// 将所有图片ID随机“排序”
	NSInteger i = [aplayImageValues count];
	while(--i > 0)
	{
		NSInteger j = arc4random() % (i+1);
		[aplayImageValues exchangeObjectAtIndex:i withObjectAtIndex:j];
	}
	return [aplayImageValues copy];
}
 ...
 */
NSArray* agetPlayImages(int size)
{
	// 随机获取size个图片ID组成的集合
	NSArray* aresourceValues = agetPlayValues(size);
	NSMutableArray* result = [[NSMutableArray alloc] init];
	// 遍历每个图片ID
	for (NSString* value in aresourceValues)
	{
		// 加载图片
